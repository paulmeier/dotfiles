...
freshly-cloned machine — especially the work box (=S427544=) — still
self-identifies before =local.el= exists. A value set in =local.el=
always wins; the hostname table is only the fallback.
#+begin_src elisp
(dolist (entry my/local-vars)
  (let ((sym (nth 0 entry))
        (default (nth 1 entry)))
...
(unless (my/non-empty my-location)
  (setq my-location
        (cond
         ...
         ((member (system-name) '("Pauls-MacBook-Air.local" "MacBook-Pro.local")) "home")
         ((member (system-name) '("chrono")) "desktop")
         (t ""))))
...
=~/.config/work/my-work.el=. Those files are gitignored ([[*Per-machine config (=local.el=)][see =local.el=]]) and
loaded here only when present, keyed off [[*Per-machine config (=local.el=)][=my-location=]].

This is the counterpart to =local.el=: =local.el= holds *values* (paths,
keys) declaratively, while this loads private *code* for the active machine.
On a machine with no such file (the common case) the block is a no-op.
...
** Fonts
~Noto Sans Symbols 2~ fixes unicode boxes for org bullets. On Arch /
...
** Frame
Start maximized and undecorated. Under Hyprland (Wayland) the compositor
tiles and draws its own borders, so =undecorated= drops Emacs' redundant
title bar and =maximized= is effectively handled by the WM anyway.
#+begin_src elisp
...
** Jinja2 templates
Jinja2-templated files — including =.j2.yml= / =.j2.yaml= (Ansible, Helm,
Salt) — open in =jinja2-mode= (package declared in =packages.el=). Plain
=.yml= / =.yaml= are intentionally left to Doom's =:lang yaml +tree-sitter=
mode, so this only claims the =.j2= variants.
#+begin_src elisp
...
auto-tangle (e.g. =zshrc.org=). The per-machine private sources are listed
too so that, on the machine where they exist (e.g. =zshrc-work.org= /
=my-work.org= on the work box), editing them re-tangles the loadable output.
Listing a file that doesn't exist on this machine is harmless — the hook
only fires when such a file is actually saved.
#+begin_src elisp
...
does *not* deliver =Alt= as =Meta= -- =M-x= (and other =M-= chords) silently
do nothing. Use the native Wayland (PGTK) build instead. On Arch/CachyOS:
#+begin_example
